...
class AD_ScatteringImage(object):
    ...
    def reduce(self):
        ...
        if (
            self.image is None
        ):  # TODO: make this conditional on need to reduce image data
        ...
        Q = (4 * math.pi / self.image.wavelength) * numpy.sin(
            0.5 * numpy.arctan2(radii, self.image.SDD)
        )
        ...
        rAvg = calc.remove_masked_data(
            rAvg, rAvg.mask
        )  # always remove the masked array last

        full = dict(Q=Q, R=rAvg, x=radii)
        self.reduced = dict(
            full=full
        )  # reset the entire dictionary with new "full" reduction
...
def command_line_interface():
    ...
    scan = reduce_area_detector_data(
        cmd_args.hdf5_file,
        cmd_args.num_bins,
