...
def get_USAXS_FlyScan_Data(scan_obj):
    ...
    try:
        ...
        return (
            None  # file may not be available yet for reading if fly scan is still going
        )
...
def get_USAXS_uascan_ScanData(scan, ar_center=None):
    ...
    usaxs["title"] = "S{} {}".format(scan.scan_number, title)
    return usaxs
...
def main(n=None, cp=False):
    ...
    local_plot = os.path.join(
        localConfig.LOCAL_WWW_LIVEDATA_DIR, localConfig.LOCAL_PLOTFILE
    )
